...
tpm[j, 2, 5] <- exp( eps_one[1, 2, j] ) #-----------------------------------|A
tpm[j, 3, 5] <- exp(                    eps_one[2, 1, j] ) #----------------|B
...
rdm[j, 3, 5] <- exp(             delta[1, 1] ) #-----------------------|OS = B
rdm[j, 4, 5] <- 0 #----------------------------------------------------|OS = C
rdm[j, 5, 5] <- exp( rho[1, j] + delta[1, 1] ) #-----------------------|OS = AB
rdm[j, 6, 5] <- 0 #----------------------------------------------------|OS = BC
rdm[j, 7, 5] <- 0 #----------------------------------------------------|OS = AC
rdm[j, 8, 5] <- 0 #----------------------------------------------------|OS = ABC
# TS = BC
...
# base colonization
gam[i, j] <-     inprod( b0[i, ], gam_cov[j, ]) + inprod( b[i, ], gam_cov[j, ] ) 
# base extinction
eps[i, j] <-     inprod( d0[i, ], gam_cov[j, ]) + inprod( d[i, ], eps_cov[j, ] ) 
...
#  is pi[species of interest, species conditioning on]. 
  for(k in 1:n_inxs){
    ...
    gam_one[rows_vec[k], cols_vec[k], j] <- 
      inprod( b0[rows_vec[k], ], pi_cov[j, ] ) + pi[rows_vec[k], cols_vec[k], j]
    # linear predictor for extinction | one species present at t-1
    eps_one[rows_vec[k], cols_vec[k], j] <- 
      inprod( d0[rows_vec[k], ], tau_cov[j, ] ) + tau[rows_vec[k], cols_vec[k], j]
  }
# make linear predictors for eps and gam when 2 species are present
  gam_two[1, j] <- inprod( b0[1, ], pi_cov[j, ] ) + pi[1,2,j] + pi[1,3,j]
  gam_two[2, j] <- inprod( b0[2, ], pi_cov[j, ] ) + pi[2,1,j] + pi[2,3,j]
  gam_two[3, j] <- inprod( b0[3, ], pi_cov[j, ] ) + pi[3,1,j] + pi[3,2,j]
# make linear predictors for eps and gam when 2 species are present
  eps_two[1, j] <- inprod( d0[1, ], tau_cov[j, ] ) + tau[1,2,j] + tau[1,3,j]
  eps_two[2, j] <- inprod( d0[2, ], tau_cov[j, ] ) + tau[2,1,j] + tau[2,3,j]
  eps_two[3, j] <- inprod( d0[3, ], tau_cov[j, ] ) + tau[3,1,j] + tau[3,2,j]
...
######
  for(i in 1:nspec){
    ...
    for( psip in 1:ncov_psi ){
      a[i, psip] ~ dnorm(0, 1.83)
    }
    # Colonization
    for( gamp in 1:ncov_gam ){
      b[i, gamp] ~ ddexp(0, lambda[i])
    }  
      b0[i, 1] ~ dnorm(0, 1.83)
      b0[i, 2] ~ ddexp(0, lambda[i])
    # Extinction
    for( epsp in 1:ncov_eps ){
      d[i, epsp] ~ ddexp(0, lambda[i])
    }  
      d0[i, 1] ~ dnorm(0, 1.83)
      d0[i, 2] ~ ddexp(0, lambda[i])
    # Detection
    for( rhop in 1:ncov_rho ){
      f[i, rhop] ~ dnorm(0, 1.83)
    }
      lambda[i] ~ dunif(0.001, 10)
  }
  for( k in 1:n_inxs ){
    # Inxs on colonization
    for( pip in 1:ncov_pi ){
      g[k, pip] ~ ddexp(0, lambda[rows_vec[k]])
    }
    # Inxs on extinction
    for( taup in 1:ncov_tau ){
      h[k, taup] ~ ddexp(0, lambda[rows_vec[k]])
    }
  }
  # Inxs on detection
  for( delp in 1:ncov_delta ){
    l[1, delp] ~ dnorm(0, 1.83)
    l[2, delp] ~ dnorm(0, 1.83)
  }
}