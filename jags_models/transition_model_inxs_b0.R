...
#  is pi[species of interest, species conditioning on]. 
  for(k in 1:n_inxs){
    ...
    gam_one[rows_vec[k], cols_vec[k], j] <- 
      inprod( b0[rows_vec[k], ], gam_cov[j, ] ) + pi[rows_vec[k], cols_vec[k], j]
    # linear predictor for extinction | one species present at t-1
    eps_one[rows_vec[k], cols_vec[k], j] <- 
      inprod( d0[rows_vec[k], ], eps_cov[j, ] ) + tau[rows_vec[k], cols_vec[k], j]
  }
# make linear predictors for eps and gam when 2 species are present
  gam_two[1, j] <- inprod( b0[1, ], gam_cov[j, ] ) + pi[1,2,j] + pi[1,3,j]
  gam_two[2, j] <- inprod( b0[2, ], gam_cov[j, ] ) + pi[2,1,j] + pi[2,3,j]
  gam_two[3, j] <- inprod( b0[3, ], gam_cov[j, ] ) + pi[3,1,j] + pi[3,2,j]
# make linear predictors for eps and gam when 2 species are present
  eps_two[1, j] <- inprod( d0[1, ], eps_cov[j, ] ) + tau[1,2,j] + tau[1,3,j]
  eps_two[2, j] <- inprod( d0[2, ], eps_cov[j, ] ) + tau[2,1,j] + tau[2,3,j]
  eps_two[3, j] <- inprod( d0[3, ], eps_cov[j, ] ) + tau[3,1,j] + tau[3,2,j]
