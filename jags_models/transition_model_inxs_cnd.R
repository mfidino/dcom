...
gam_a_b[j] <-  ilogit( inprod(g[1, ], pi_cov[j, ]) ) # a|b
# colonization species a inxs | c
gam_a_c[j] <-  ilogit( inprod(g[2, ], pi_cov[j, ]) ) # a|c
# colonization species a inxs | bc
gam_a_bc[j] <- gam_a_b[j] * gam_a_c[j]
# colonization species b inxs | a
gam_b_a[j] <-  ilogit( inprod(g[3, ], pi_cov[j, ]) ) # b|a
# colonization species b inxs | c
gam_b_c[j] <-  ilogit( inprod(g[4, ], pi_cov[j, ]) ) # b|c
# colonization species b inxs | ac
gam_b_ac[j] <- gam_b_a[j] * gam_b_c[j]
# colonization species c inxs | a
gam_c_a[j] <-  ilogit( inprod(g[5, ], pi_cov[j, ]) ) # c|a
# colonization species c inxs | b
gam_c_b[j] <-  ilogit( inprod(g[6, ], pi_cov[j, ]) ) # c|b
# colonization species c inxs | ab
gam_c_ab[j] <- gam_c_a[j] * gam_c_b[j]
# base extinction
...
eps_a_b[j] <-  ilogit( inprod(h[1, ], tau_cov[j, ]) ) # a|b
# extinction species a inxs | c
eps_a_c[j] <-  ilogit( inprod(h[2, ], tau_cov[j, ]) ) # a|c
# extinction species a inxs | bc
eps_a_bc[j] <- eps_a_b[j] * eps_a_c[j]
# extinction species b inxs | a
eps_b_a[j] <-  ilogit( inprod(h[3, ], tau_cov[j, ]) ) # b|a
# extinction species b inxs | c
eps_b_c[j] <-  ilogit( inprod(h[4, ], tau_cov[j, ]) ) # b|c
# extinction species b inxs | ac
eps_b_ac[j] <- eps_b_a[j] * eps_b_c[j]
# extinction species c inxs | a
eps_c_a[j] <-  ilogit( inprod(h[5, ], tau_cov[j, ]) ) # c|a
# extinction species c inxs | b
eps_c_b[j] <-  ilogit( inprod(h[6, ], tau_cov[j, ]) ) # c|b
# extinction species c inxs | ab
eps_c_ab[j] <- eps_c_a[j] * eps_c_b[j]
...
rho_b_a[j] <- ilogit( inprod(l[1, ], delta_cov[j,]) )
# species c detection | a
rho_c_a[j] <- ilogit( inprod(l[2, ], delta_cov[j,]) )
