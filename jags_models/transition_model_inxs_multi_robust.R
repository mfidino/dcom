...
for(j in 1:nsite) {
  # for first season
  z[j, 1] ~ dcat( guess * pvec + (1 - guess) * mu[j,1, 1:nout] )
  mu[j,1, 1:nout] <- fsm[j, ( 1:nout )] / sum(fsm[j, ( 1:nout )])
  for(t in 2:nyear){
    z[j, t] ~ dcat( guess * pvec + (1 - guess) * mu[j,t, 1:nout] )
    mu[j,t, 1:nout] <- tpm[j, ( 1:nout ) , z[ j, t-1]] / 
               sum(tpm[j, ( 1:nout ) , z[ j, t-1]] )
  }
}
for( j in 1:nsite ) {
######
...
#  is pi[species of interest, species conditioning on]. 
  for(k in 1:n_inxs){
    ...
    gam_one[rows_vec[k], cols_vec[k], j] <-   pi[rows_vec[k], cols_vec[k], j]
    # linear predictor for extinction | one species present at t-1
    eps_one[rows_vec[k], cols_vec[k], j] <-  tau[rows_vec[k], cols_vec[k], j]
  }
# make linear predictors for eps and gam when 2 species are present
  gam_two[1, j] <- pi[1,2,j] + pi[1,3,j]
  gam_two[2, j] <- pi[2,1,j] + pi[2,3,j]
  gam_two[3, j] <- pi[3,1,j] + pi[3,2,j]
# make linear predictors for eps and gam when 2 species are present
  eps_two[1, j] <- tau[1,2,j] + tau[1,3,j]
  eps_two[2, j] <- tau[2,1,j] + tau[2,3,j]
  eps_two[3, j] <- tau[3,1,j] + tau[3,2,j]
  # coyote influence on detection for opossum and raccoon
  delta[1, j]  <-  rho[2, j] + inprod( l[1, ], delta_cov[j, ] )
  delta[2, j]  <-  rho[3, j] + inprod( l[2, ], delta_cov[j, ] )
